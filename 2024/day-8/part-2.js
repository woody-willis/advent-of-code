...
for (let i = 0; i < frequencies.length; i++) {
    ...
    for (const antennaLocation of antennaLocations) {
        ...
        for (let j = 0; j < locationDifferences.length; j++) {
            for (let multiplier = 1; multiplier <= map.length; multiplier++) {
                const antinodeX = x + locationDifferences[j][0]*multiplier;
                const antinodeY = y + locationDifferences[j][1]*multiplier;

                if (antinodeX < 0 || antinodeX >= map[0].length || antinodeY < 0 || antinodeY >= map.length) break;
